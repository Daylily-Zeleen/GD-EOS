...
如果你使用预编译插件（或自行编译），GD-EOS 内置了一个 `EditorExportPlugin`（`EOSAndroidExportPlugin`）来自动完成下述的安卓导出步骤。这是默认且推荐的安卓导出方式。你只需要：
...
以下手动步骤仅作为参考保留，或用于不使用 GD-EOS 导出插件的情况。如果你使用插件（默认行为），则无需遵循这些步骤。
