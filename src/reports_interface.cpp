...
Signal IEOS::reports_interface_send_player_behavior_report(Ref<RefCounted> p_options) {
    ...
    EOS_Reports_SendPlayerBehaviorReport(s_reportsInterface, &options, (void *)*p_options, [](const EOS_Reports_SendPlayerBehaviorReportCompleteCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("reports_interface_send_player_behavior_report_callback", ref_ret);
    });

    return SIGNAL("reports_interface_send_player_behavior_report_callback");
}
