...
Signal IEOS::user_info_interface_query_user_info(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_UserInfo_QueryUserInfo(s_userInfoInterface, &options, (void *)*p_options, [](const EOS_UserInfo_QueryUserInfoCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("user_info_interface_query_user_info_callback", ref_ret);
    });

    return SIGNAL("user_info_interface_query_user_info_callback");
}

Signal IEOS::user_info_interface_query_user_info_by_display_name(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_UserInfo_QueryUserInfoByDisplayName(s_userInfoInterface, &options, (void *)*p_options, [](const EOS_UserInfo_QueryUserInfoByDisplayNameCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("user_info_interface_query_user_info_by_display_name_callback", ref_ret);
    });

    return SIGNAL("user_info_interface_query_user_info_by_display_name_callback");
}

Signal IEOS::user_info_interface_query_user_info_by_external_account(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_UserInfo_QueryUserInfoByExternalAccount(s_userInfoInterface, &options, (void *)*p_options, [](const EOS_UserInfo_QueryUserInfoByExternalAccountCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("user_info_interface_query_user_info_by_external_account_callback", ref_ret);
    });

    return SIGNAL("user_info_interface_query_user_info_by_external_account_callback");
}
...
EOS_OnlinePlatformType IEOS::user_info_interface_get_local_platform_type(Ref<RefCounted> p_options) {
    EOS_UserInfo_GetLocalPlatformTypeOptions options;
    memset(&options, 0, sizeof(options));
    options.ApiVersion = EOS_USERINFO_GETLOCALPLATFORMTYPE_API_LATEST;

    return static_cast<EOS_OnlinePlatformType>(EOS_UserInfo_GetLocalPlatformType(s_userInfoInterface, &options));
}
