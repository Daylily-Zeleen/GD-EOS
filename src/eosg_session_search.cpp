...
EOS_EResult EOSGSessionSearch::remove_parameter(const String &p_key, int p_comparison_op) {
    ...
    return static_cast<EOS_EResult>(ret);
}

EOS_EResult EOSGSessionSearch::set_max_results(int p_max_results) {
...
EOS_EResult EOSGSessionSearch::set_parameter(const String &p_key, Variant p_value, int p_comparison_op) {
    CharString key = p_key.utf8();
    ...
    return static_cast<EOS_EResult>(EOS_SessionSearch_SetParameter(m_internal, &options));
}

EOS_EResult EOSGSessionSearch::set_session_id(const String &p_session_id) {
...
EOS_EResult EOSGSessionSearch::set_target_user_id(const String &p_target_user_id) {
...
Signal EOSGSessionSearch::find(const String &p_local_user_id) {
    ...
    EOS_SessionSearch_Find(m_internal, &options, nullptr, [](const EOS_SessionSearch_FindCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("session_search_find_callback", ref_ret);
    });

    return Signal(IEOS::get_singleton(), "session_search_find_callback");
}