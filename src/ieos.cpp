...
#define IEOS_BIND_SIGNAL(m_name) ADD_SIGNAL(MethodInfo(#m_name, MAKE_DATA_CLASS_PROP_INFO_BY_SIGNAL_NAME(m_name)))
...
void IEOS::_bind_methods() {
    ...
    IEOS_BIND_METHOD(platform_interface_set_tick_by_scene_tree);
    IEOS_BIND_METHOD(platform_interface_is_tick_by_scene_tree);
    IEOS_BIND_METHOD(platform_interface_tick);
    ...
    ADD_SIGNAL(MethodInfo("p2p_interface_query_nat_type_callback", PropertyInfo(Variant::INT, "nat_type"))); // TODO: 使用Wrapper
    ...
    IEOS_BIND_SIGNAL(auth_interface_login_status_changed_callback);
    ...
    IEOS_BIND_SIGNAL(connect_interface_auth_expiration_callback);
    ...
    IEOS_BIND_SIGNAL(connect_interface_login_status_changed_callback);
    ...
    IEOS_BIND_SIGNAL(p2p_interface_incoming_packet_queue_full_callback);
    ...
    IEOS_BIND_SIGNAL(rtc_audio_interface_audio_before_render_callback);
    IEOS_BIND_SIGNAL(rtc_audio_interface_audio_before_send_callback);
    IEOS_BIND_SIGNAL(rtc_audio_interface_audio_devices_changed_callback);
    IEOS_BIND_SIGNAL(rtc_audio_interface_audio_input_state_callback);
    IEOS_BIND_SIGNAL(rtc_audio_interface_audio_output_state_callback);
    ...
    IEOS_BIND_SIGNAL(rtc_audio_interface_participant_updated_callback);
    IEOS_BIND_SIGNAL(rtc_interface_block_participant_callback);
    IEOS_BIND_SIGNAL(rtc_interface_disconnected_callback);
    IEOS_BIND_SIGNAL(rtc_interface_join_room_callback);
    IEOS_BIND_SIGNAL(rtc_interface_leave_room_callback);
    IEOS_BIND_SIGNAL(rtc_interface_participant_status_changed_callback);
    IEOS_BIND_SIGNAL(rtc_interface_room_statistics_updated_callback);
...
void IEOS::_notification(int p_what) {
    if (p_what == NOTIFICATION_POSTINITIALIZE && !Engine::get_singleton()->is_editor_hint()) {
        callable_mp(this, &IEOS::platform_interface_set_tick_by_scene_tree).call_deferred(true);
    }
}
...
void IEOS::platform_tick_internal() {
    if (IEOS::get_singleton()->s_platformInterface != nullptr) {
        EOS_Platform_Tick(IEOS::get_singleton()->s_platformInterface);
    }
}

bool IEOS::platform_interface_is_tick_by_scene_tree() {
    return platform_tick_by_scene_tree;
}

void IEOS::platform_interface_set_tick_by_scene_tree(bool p_enabled) {
    if (auto scene_tree = cast_to<SceneTree>(Engine::get_singleton()->get_main_loop())) {
        Callable cb = callable_mp(this, &IEOS::platform_tick_internal);
        StringName sn_process_frame{ "process_frame" };
        if (p_enabled && !scene_tree->is_connected(sn_process_frame, cb)) {
            scene_tree->connect(sn_process_frame, cb);
        } else if (!p_enabled && scene_tree->is_connected(sn_process_frame, cb)) {
            scene_tree->disconnect(sn_process_frame, cb);
        }
        platform_tick_by_scene_tree = p_enabled;
    } else {
        platform_tick_by_scene_tree = false;
        ERR_FAIL_COND_MSG(p_enabled, "Can't get SceneTree by \"Engine::get_singleton()->get_main_loop()\"");
    }
}

void IEOS::platform_interface_tick() {
    ERR_FAIL_COND(platform_tick_by_scene_tree);
    platform_tick_internal();
}