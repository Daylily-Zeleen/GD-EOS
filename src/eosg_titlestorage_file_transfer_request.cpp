...
EOS_EResult EOSGTitleStorageFileTransferRequest::get_file_request_state() {
    ...
    return static_cast<EOS_EResult>(EOS_TitleStorageFileTransferRequest_GetFileRequestState(m_internal));
}
...
EOS_EResult EOSGTitleStorageFileTransferRequest::cancel_request() {
    ERR_FAIL_NULL_V_MSG(m_internal, static_cast<EOS_EResult>(EOS_EResult::EOS_InvalidState), "The object has not been initialized by EOS.");
    return static_cast<EOS_EResult>(EOS_TitleStorageFileTransferRequest_CancelRequest(m_internal));
}