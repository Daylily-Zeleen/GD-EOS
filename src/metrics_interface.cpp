...
EOS_EResult IEOS::metrics_interface_begin_player_session(Ref<RefCounted> p_options) {
    ...
    return static_cast<EOS_EResult>(EOS_Metrics_BeginPlayerSession(s_metricsInterface, &options));
}

EOS_EResult IEOS::metrics_interface_end_player_session(Ref<RefCounted> p_options) {
    CharString account_id = VARIANT_TO_CHARSTRING(p_options->get("account_id"));
    ...
    return static_cast<EOS_EResult>(EOS_Metrics_EndPlayerSession(s_metricsInterface, &options));
}
