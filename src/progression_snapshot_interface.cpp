...
EOS_EResult IEOS::progression_snapshot_interface_add_progression(Ref<RefCounted> p_options) {
    ...
    return static_cast<EOS_EResult>(EOS_ProgressionSnapshot_AddProgression(s_progressionSnapshotInterface, &options));
}

Signal IEOS::progression_snapshot_interface_submit_snapshot(Ref<RefCounted> p_options) {
    ...
    EOS_ProgressionSnapshot_SubmitSnapshot(s_progressionSnapshotInterface, &options, (void *)*p_options, [](const EOS_ProgressionSnapshot_SubmitSnapshotCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("progression_snapshot_interface_submit_snapshot_callback", ref_ret);
    });

    return SIGNAL("progression_snapshot_interface_submit_snapshot_callback");
}

Signal IEOS::progression_snapshot_interface_delete_snapshot(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_ProgressionSnapshot_DeleteSnapshot(s_progressionSnapshotInterface, &options, (void *)*p_options, [](const EOS_ProgressionSnapshot_DeleteSnapshotCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("progression_snapshot_interface_delete_snapshot_callback", ref_ret);
    });

    return SIGNAL("progression_snapshot_interface_delete_snapshot_callback");
}

EOS_EResult IEOS::progression_snapshot_interface_end_snapshot(Ref<RefCounted> p_options) {
    EOS_ProgressionSnapshot_EndSnapshotOptions options;
    memset(&options, 0, sizeof(options));
    options.ApiVersion = EOS_PROGRESSIONSNAPSHOT_ENDSNAPSHOT_API_LATEST;
    options.SnapshotId = static_cast<uint32_t>(static_cast<int>(p_options->get("snapshot_id")));

    return static_cast<EOS_EResult>(EOS_ProgressionSnapshot_EndSnapshot(s_progressionSnapshotInterface, &options));
}
