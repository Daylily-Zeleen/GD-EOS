...
Signal IEOS::mods_interface_enumerate_mods(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_Mods_EnumerateMods(s_modsInterface, &options, (void *)*p_options, [](const EOS_Mods_EnumerateModsCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("mods_interface_enumerate_mods_callback", ref_ret);
    });

    return SIGNAL("mods_interface_enumerate_mods_callback");
}

Signal IEOS::mods_interface_install_mod(Ref<RefCounted> p_options) {
    ...
    EOS_Mods_InstallMod(s_modsInterface, &options, (void *)*p_options, [](const EOS_Mods_InstallModCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("mods_interface_install_mod_callback", ref_ret);
    });

    return SIGNAL("mods_interface_enumerate_mods_callback");
}

Signal IEOS::mods_interface_uninstall_mod(Ref<RefCounted> p_options) {
    Dictionary p_mod = p_options->get("mod");
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_Mods_UninstallMod(s_modsInterface, &options, (void *)*p_options, [](const EOS_Mods_UninstallModCallbackInfo *data) {
        ...
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        ret["mod"] = eosg_mods_mod_identifier_to_dict(data->Mod);
        IEOS::get_singleton()->emit_signal("mods_interface_uninstall_mod_callback", ref_ret);
    });

    return SIGNAL("mods_interface_enumerate_mods_callback");
}

Signal IEOS::mods_interface_update_mod(Ref<RefCounted> p_options) {
    Dictionary p_mod = p_options->get("mod");
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_Mods_UpdateMod(s_modsInterface, &options, (void *)*p_options, [](const EOS_Mods_UpdateModCallbackInfo *data) {
        ...
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        ret["mod"] = eosg_mods_mod_identifier_to_dict(data->Mod);
        IEOS::get_singleton()->emit_signal("mods_interface_update_mod_callback", ref_ret);
    });
