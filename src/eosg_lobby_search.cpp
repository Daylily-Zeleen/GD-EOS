...
Signal EOSGLobbySearch::find(const String &p_local_user_id) {
    ...
    EOS_LobbySearch_Find(m_internal, &options, nullptr, [](const EOS_LobbySearch_FindCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("lobby_search_find_callback", ref_ret);
    });

    return Signal(IEOS::get_singleton(), "lobby_search_find_callback");
}

EOS_EResult EOSGLobbySearch::set_lobby_id(const String &p_lobby_id) {
    ...
    return static_cast<EOS_EResult>(EOS_LobbySearch_SetLobbyId(m_internal, &options));
}

EOS_EResult EOSGLobbySearch::set_target_user_id(const String &p_target_user_id) {
    ...
    return static_cast<EOS_EResult>(EOS_LobbySearch_SetTargetUserId(m_internal, &options));
}

EOS_EResult EOSGLobbySearch::set_parameter(const String &p_key, Variant p_value, int p_comparison_op) {
    ...
    return static_cast<EOS_EResult>(EOS_LobbySearch_SetParameter(m_internal, &options));
}

EOS_EResult EOSGLobbySearch::remove_parameter(const String &p_key, int p_comparison_op) {
    CharString key = p_key.utf8();
    ...
    return static_cast<EOS_EResult>(EOS_LobbySearch_RemoveParameter(m_internal, &options));
}

EOS_EResult EOSGLobbySearch::set_max_results(int p_max_results) {
    ...
    return static_cast<EOS_EResult>(EOS_LobbySearch_SetMaxResults(m_internal, &options));
}
