...
Signal IEOS::p2p_interface_query_nat_type() {
    ...
    EOS_P2P_QueryNATType(s_p2pInterface, &options, nullptr, [](const EOS_P2P_OnQueryNATTypeCompleteInfo *data) {
        ...
        ret["result_code"] = data->ResultCode;
        ret["nat_type"] = data->NATType;
        get_singleton()->emit_signal("p2p_interface_query_nat_type_callback", ref_ret);
    });

    return SIGNAL("p2p_interface_query_nat_type_callback");
}

EOS_ENATType IEOS::p2p_interface_get_nat_type() {
    ...
    return static_cast<EOS_ENATType>(nat_type);
}

void IEOS::p2p_interface_set_relay_control(EOS_ERelayControl control) {
...
EOS_ERelayControl IEOS::p2p_interface_get_relay_control() {
    ...
    return static_cast<EOS_ERelayControl>(relay_control);
}
