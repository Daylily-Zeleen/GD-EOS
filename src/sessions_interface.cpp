...
EOS_EResult IEOS::sessions_interface_dump_session_state(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    return static_cast<EOS_EResult>(EOS_Sessions_DumpSessionState(s_sessionsInterface, &options));
}
...
EOS_EResult IEOS::sessions_interface_is_user_in_session(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
...
Signal IEOS::sessions_interface_destroy_session(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_DestroySession(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_DestroySessionCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("sessions_interface_destroy_session_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_destroy_session_callback");
}

Signal IEOS::sessions_interface_end_session(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_EndSession(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_EndSessionCallbackInfo *data) {
        ...
        client_data->unreference();
        ret["client_data"] = client_data->get("client_data");
        IEOS::get_singleton()->emit_signal("sessions_interface_end_session_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_end_session_callback");
}

Signal IEOS::sessions_interface_join_session(Ref<RefCounted> p_options) {
    Ref<EOSGSessionDetails> session_details = Object::cast_to<EOSGSessionDetails>(p_options->get("session_details"));
    ERR_FAIL_NULL_V_MSG(session_details, SIGNAL("sessions_interface_join_session_callback"), "Error joining session. JoinSessionOptions.session_details is null.");
    ERR_FAIL_NULL_V_MSG(session_details->get_internal(), SIGNAL("sessions_interface_join_session_callback"), "Error joining session. EOSGSessionDetails is null.");
    ...
    EOS_Sessions_JoinSession(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_JoinSessionCallbackInfo *data) {
        ...
        client_data->unreference();
        ret["client_data"] = client_data->get("client_data");
        IEOS::get_singleton()->emit_signal("sessions_interface_join_session_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_join_session_callback");
}

Signal IEOS::sessions_interface_query_invites(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_Sessions_QueryInvites(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_QueryInvitesCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("sessions_interface_query_invites_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_query_invites_callback");
}

Signal IEOS::sessions_interface_register_players(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_RegisterPlayers(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_RegisterPlayersCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("sessions_interface_register_players_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_register_players_callback");
}

Signal IEOS::sessions_interface_reject_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString invite_id = VARIANT_TO_CHARSTRING(p_options->get("invite_id"));
    ...
    EOS_Sessions_RejectInvite(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_RejectInviteCallbackInfo *data) {
        ...
        client_data->unreference();
        ret["client_data"] = client_data->get("client_data");
        IEOS::get_singleton()->emit_signal("sessions_interface_reject_invite_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_reject_invite_callback");
}

Signal IEOS::sessions_interface_send_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_SendInvite(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_SendInviteCallbackInfo *data) {
        ...
        client_data->unreference();
        ret["client_data"] = client_data->get("client_data");
        IEOS::get_singleton()->emit_signal("sessions_interface_send_invite_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_send_invite_callback");
}

Signal IEOS::sessions_interface_start_session(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_StartSession(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_StartSessionCallbackInfo *data) {
        ...
        client_data->unreference();
        ret["client_data"] = client_data->get("client_data");
        IEOS::get_singleton()->emit_signal("sessions_interface_start_session_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_start_session_callback");
}

Signal IEOS::sessions_interface_unregister_players(Ref<RefCounted> p_options) {
    CharString session_name = VARIANT_TO_CHARSTRING(p_options->get("session_name"));
    ...
    EOS_Sessions_UnregisterPlayers(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_UnregisterPlayersCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("sessions_interface_unregister_players_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_unregister_players_callback");
}

Signal IEOS::sessions_interface_update_session(Ref<RefCounted> p_options) {
    ...
    ERR_FAIL_NULL_V_MSG(session_modification, SIGNAL("sessions_interface_update_session_callback"), "Error updating session. UpdateSessionOptions.session_modification is null.");
    ERR_FAIL_NULL_V_MSG(session_modification->get_internal(), SIGNAL("sessions_interface_update_session_callback"), "Error updating session. EOSGSessionModification is null.");
    ...
    EOS_Sessions_UpdateSession(s_sessionsInterface, &options, (void *)*p_options, [](const EOS_Sessions_UpdateSessionCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("sessions_interface_update_session_callback", ref_ret);
    });

    return SIGNAL("sessions_interface_update_session_callback");
}
