...
Signal IEOS::presence_interface_query_presence(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_Presence_QueryPresence(s_presenceInterface, &options, (void *)*p_options, [](const EOS_Presence_QueryPresenceCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("presence_interface_query_presence_callback", ref_ret);
    });

    return SIGNAL("presence_interface_query_presence_callback");
}

Signal IEOS::presence_interface_set_presence(Ref<RefCounted> p_options) {
    ...
    ERR_FAIL_NULL_V_MSG(presence_modification, SIGNAL("presence_interface_set_presence_callback"), "Error setting presence. SetPresenceOptions.presence_modification is null.");
    ERR_FAIL_NULL_V_MSG(presence_modification->get_internal(), SIGNAL("presence_interface_set_presence_callback"), "Error setting presence. EOSGPresenceModification is null.");
    ...
    EOS_Presence_SetPresence(s_presenceInterface, &options, (void *)*p_options, [](const EOS_Presence_SetPresenceCallbackInfo *data) {
        ...
        ret["client_data"] = client_data->get("client_data");
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        IEOS::get_singleton()->emit_signal("presence_interface_set_presence_callback", ref_ret);
    });

    return SIGNAL("presence_interface_set_presence_callback");
}
