...
EOS_EResult IEOS::custom_invites_interface_finalize_invite(Ref<RefCounted> p_options) {
    ...
    return static_cast<EOS_EResult>(EOS_CustomInvites_FinalizeInvite(s_customInvitesInterface, &options));
}

Signal IEOS::custom_invites_interface_send_custom_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_CustomInvites_SendCustomInvite(s_customInvitesInterface, &options, (void *)*p_options, [](const EOS_CustomInvites_SendCustomInviteCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("custom_invites_interface_send_custom_invite_callback", ref_ret);
    });

    return SIGNAL("custom_invites_interface_send_custom_invite_callback");
}

EOS_EResult IEOS::custom_invites_interface_set_custom_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    return static_cast<EOS_EResult>(EOS_CustomInvites_SetCustomInvite(s_customInvitesInterface, &options));
}

Signal IEOS::custom_invites_interface_send_request_to_join(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_CustomInvites_SendRequestToJoin(s_customInvitesInterface, &options, (void *)*p_options, [](const EOS_CustomInvites_SendRequestToJoinCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("custom_invites_interface_send_request_to_join_callback", ref_ret);
    });

    return SIGNAL("custom_invites_interface_send_request_to_join_callback");
}
...
Signal IEOS::custom_invites_interface_accept_request_to_join(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_CustomInvites_AcceptRequestToJoin(s_customInvitesInterface, &options, (void *)*p_options, [](const EOS_CustomInvites_AcceptRequestToJoinCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("custom_invites_interface_accept_request_to_join_callback", ref_ret);
    });

    return SIGNAL("custom_invites_interface_accept_request_to_join_callback");
}

Signal IEOS::custom_invites_interface_reject_request_to_join(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_CustomInvites_RejectRequestToJoin(s_customInvitesInterface, &options, (void *)*p_options, [](const EOS_CustomInvites_RejectRequestToJoinCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("custom_invites_interface_reject_request_to_join_callback", ref_ret);
    });

    return SIGNAL("custom_invites_interface_reject_request_to_join_callback");
}