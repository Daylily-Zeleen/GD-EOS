 ...
 ****************************************/
void EOSGPacketPeerMediator::_on_connect_interface_login(const Ref<EOSDataClass> &data) {
    int result_code = (*data.ptr())["result_code"];
    if (static_cast<EOS_EResult>(result_code) != EOS_EResult::EOS_Success) {
        return;
    }
    String local_user_id = (*data.ptr())["local_user_id"];
    if (local_user_id != "") {
