...
Signal IEOS::friends_interface_accept_invite(Ref<RefCounted> p_options) {
    ...
    EOS_Friends_AcceptInvite(s_friendsInterface, &options, (void *)*p_options, [](const EOS_Friends_AcceptInviteCallbackInfo *data) {
        ...
        IEOS::get_singleton()->emit_signal("friends_interface_accept_invite_callback", ref_ret);
    });

    return SIGNAL("friends_interface_accept_invite_callback");
}
...
EOS_EFriendsStatus IEOS::friends_interface_get_status(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    return static_cast<EOS_EFriendsStatus>(EOS_Friends_GetStatus(s_friendsInterface, &options));
}

Signal IEOS::friends_interface_query_friends(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    ...
    EOS_Friends_QueryFriends(s_friendsInterface, &options, (void *)*p_options, [](const EOS_Friends_QueryFriendsCallbackInfo *data) {
        ...
        ret["client_data"] = client_data->get("client_data");
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        IEOS::get_singleton()->emit_signal("friends_interface_query_friends_callback", ref_ret);
    });

    return SIGNAL("friends_interface_query_friends_callback");
}

Signal IEOS::friends_interface_reject_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_Friends_RejectInvite(s_friendsInterface, &options, (void *)*p_options, [](const EOS_Friends_RejectInviteCallbackInfo *data) {
        ...
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        ret["target_user_id"] = eosg_epic_account_id_to_string(data->TargetUserId);
        IEOS::get_singleton()->emit_signal("friends_interface_reject_invite_callback", ref_ret);
    });

    return SIGNAL("friends_interface_reject_invite_callback");
}

Signal IEOS::friends_interface_send_invite(Ref<RefCounted> p_options) {
    CharString local_user_id = VARIANT_TO_CHARSTRING(p_options->get("local_user_id"));
    CharString target_user_id = VARIANT_TO_CHARSTRING(p_options->get("target_user_id"));
    ...
    EOS_Friends_SendInvite(s_friendsInterface, &options, (void *)*p_options, [](const EOS_Friends_SendInviteCallbackInfo *data) {
        ...
        ret["local_user_id"] = eosg_epic_account_id_to_string(data->LocalUserId);
        ret["target_user_id"] = eosg_epic_account_id_to_string(data->TargetUserId);
        IEOS::get_singleton()->emit_signal("friends_interface_send_invite_callback", ref_ret);
    });

    return SIGNAL("friends_interface_send_invite_callback");
}
